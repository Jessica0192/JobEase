...
async def retrieve_jobRecord_by_id(job_record_id: int,
    ...
    db_job_record = job_record_service.get_jobRecord_by_id(db=db, job_record_id=job_record_id)
...
async def retrieve_all_jobRecords(limit: int = 100,
                                  db: Session = Depends(get_db),
                                  current_user: User = Depends(auth_service.get_current_user_from_token)):
    return job_record_service.get_all_jobRecords(current_user_id=current_user.id, db=db, limit=limit)


@router.post("/")
async def create_new_jobRecord(job_record: job_record_schema.JobRecordAll,
                               db: Session = Depends(get_db),
                               current_user: User = Depends(auth_service.get_current_user_from_token)):
    db_job_record = job_record_service.create_jobRecord(current_user_id=current_user.id, db=db, job_record=job_record)
    if db_job_record is None:
...
async def update_jobRecord(job_record_id: int,
    ...
    db_job_record = job_record_service.update_jobRecord(db=db, job_record_id=job_record_id, job_record=job_record)
