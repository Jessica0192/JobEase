    ...
    if job_record_user_id is not None:
        ...
        if deleted_resource is False:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,
                                detail="Job Record not found, so cannot be deleted")
        return {"message": "Successfully deleted"}
    raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Job Record not found, so cannot be deleted")
